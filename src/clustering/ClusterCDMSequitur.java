...
public class ClusterCDMSequitur {
	...
	public static void main(String[] args) {
		...
		String dir = args[1];
		try {
			ClusterAll.o = new PrintWriter(new BufferedWriter(new FileWriter(files + "/Cluster_CDMSequitur.csv")));
		...
		try {
			ClusterAll.f = new BufferedReader(new FileReader(dir + "/" + files));
				...
				for(double threshold = .3; threshold < .71; threshold += .02) {
	...
	public static void cluster(double threshold) {
		...
		while(!ClusterAll.sequences.isEmpty()) {
			...
			for(int i = 0; i < ClusterAll.sequences.size(); i++) {
				if(ValidateCA.cdmSequitur(representative, ClusterAll.sequences.get(i)) < threshold) {
